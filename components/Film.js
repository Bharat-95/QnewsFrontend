...
// Function to show time difference
...
const Latest = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      // Sort before filtering to ensure latest news appears first
      const sortedData = responseData
        .filter(news => news.status === "Approved" && news.category === "Film")
        .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));

      setData(sortedData);
    } catch (error) {
      console.error("Unable to fetch the Latest Film News", error);
    }
  };

  useEffect(() => {
    fetchData();
    const interval = setInterval(fetchData, 30000); // Fetch every 30 seconds for real-time updates
    return () => clearInterval(interval);
  }, []);

  // Function to load more items when scrolled to the bottom
