...
const PostViewCount = ({ postUrl }) => {
  const [viewCount, setViewCount] = useState(100); // ✅ Start from 100
  ...
  useEffect(() => {
    ...
    const fetchViewCount = async () => {
      try {
        ...
        const combinedViews = 100 + viewCountTe + viewCountEn; // ✅ Start from 100 & add Clicky count

        setViewCount(combinedViews);
      } catch (error) {
