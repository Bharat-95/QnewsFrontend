...
const Latest = () => {
  ...
  const loadMoreImages = useCallback(() => {
    if (visibleCount < data.length && visibleCount < 8) {
      setVisibleCount((prevCount) => prevCount + 1); // Increment visible count
    }
    ...
    <div className="max-w-full overflow-hidden">
      ...
      <div className="overflow-x-auto scrollbar-hide sm:w-screen lg:w-auto md:w-auto py-4">
        <div className="flex gap-4">
          {data
            .filter((latest) => latest.status === "Approved")
            .slice(0, visibleCount)
            .map((latest) => (
              <Link
                href={`/news/${latest.newsId}`}
                key={latest.newsId}
                className="flex-shrink-0 w-[80%] md:w-[40%] lg:w-[30%] border border-orange-600 lg:p-3 md:p-2 p-2 rounded-md shadow-lg space-y-2 hover:transform duration-500 hover:translate-x-2 hover:-translate-y-2"
              >
                <Image
                  alt="No Image Found"
                  src={latest.image}
