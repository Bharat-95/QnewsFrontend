import { supabaseRest, supabaseStorageUpload } from "@/lib/server/supabase";
...
export async function saveVideoThumbnail(file) {
  ...
  const filename = `video-thumbnails/${Date.now()}-${crypto.randomUUID()}.${ext}`;
  const bucket = process.env.SUPABASE_BUCKET_VIDEO_THUMBNAILS || "video-thumbnails";
